...
if ($_POST['parse_var'] == "new"){
	...
	if($update){
	$msg = "Discover Nigeria Article, <font color=\"#FF0000\">".stripslashes($title)."</font> Content has been edited. <a href=\"state.php\"><button class=\"btn btn-large btn-primary\" type=\"submit\" id=\"update\">Edit other Geography Articles</button></a>";
	}
	else{
		...
		?>
          ...
          <h3>Contents for <?php echo $title2; ?></h3>
           <div style="padding: 10px; width: 850px; font-family: Verdana, Geneva, sans-serif; font-size:13px; margin:10px 0px 10px 10px;"><?php echo $msg; ?></div>   
            ...
            <form class="form-signin" method="post" action="state-edit.php?code=<?php echo $hashcode; ?>&user=<?php echo $user; ?>">
            
            <input type="hidden" name="hashskit" id="hashskit" value="<?php echo $hashcode; ?>" />
            
             <label>Title</label><input type="text" class="input-block-level" name="title" id="title" value="<?php echo $title2; ?>" style="width:600px; padding:5px; font-size:13px;">
