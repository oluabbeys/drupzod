...
if ($_POST['parse_var'] == "new"){
	...
	$update = mysql_query("UPDATE newsletterstories SET topic='$title', story='$content', month='$month', category='$category' WHERE hash='$hashremix'");
	if($update){
	$msg = "Newsletter Article, <font color=\"#FF0000\">".stripslashes($title)."</font> Content has been edited. <a href=\"edit-newsletter.php\"><button class=\"btn btn-large btn-primary\" type=\"submit\" id=\"update\">Edit other Newsletter Articles</button></a>";
	}
	else{
...
<title>Edit Content Page - Newsletter Article</title>
    ...
    <div class="container-fluid">
      <div class="row-fluid">
        ...
        <?php
		  ...
		  $getnews = mysql_query("SELECT * FROM newsletterstories WHERE hash='$hashcode'");
		  $rownews = mysql_fetch_array($getnews);
		  $id = $rownews['id'];
		  $topic = $rownews['topic'];
		  $story = $rownews['story'];
		  $month = $rownews['month'];
		  $category = $rownews['category'];
		...
		?>
          ...
          <h3>Contents for <?php echo $topic; ?></h3>
           <div style="padding: 10px; width: 1000px; font-family: Verdana, Geneva, sans-serif; font-size:13px; margin:10px 0px 10px 10px;"><?php echo $msg; ?></div>   
            ...
            <form class="form-signin" method="post" action="newsletter-edit.php?code=<?php echo $hashcode; ?>&user=<?php echo $user; ?>">
            
            <input type="hidden" name="hashskit" id="hashskit" value="<?php echo $hashcode; ?>" />
            
             <label>Title</label><input type="text" class="input-block-level" name="title" id="title" value="<?php echo $topic; ?>" style="width:600px; padding:5px; font-size:13px;">
               
             <label>Article</label><textarea style="width:100%; height:300px;" name="content" id="content"><?php echo $story; ?></textarea>
             
             <label>Month</label><input type="text" class="input-block-level" name="month" id="month" value="<?php echo $month; ?>">
             
             <label>Category</label><input type="text"  class="input-block-level" name="category" id="category" value="<?php echo $category; ?>"/>
