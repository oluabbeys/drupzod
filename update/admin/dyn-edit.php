...
if ($_POST['parse_var'] == "new"){
	$hashremix = $_POST['hashdyn'];
	$date = $_POST['date'];
	$date = mysql_real_escape_string(stripslashes($date));
	$content = $_POST['content'];
	$content = mysql_real_escape_string(stripslashes($content));
	
	$update = mysql_query("UPDATE archives SET contents='$content', date='$date' WHERE hash='$hashremix'");
	if($update){
	$msg = "Did You Know Article, <font color=\"#FF0000\">".stripslashes($date)."</font> Content has been edited. <a href=\"edit-dyn.php\"><button class=\"btn btn-large btn-primary\" type=\"submit\" id=\"update\">Edit other Did You Know Articles</button></a>";
	}
	else{
...
<title>Edit Content Page - Did You Know Article</title>
    ...
    <div class="container-fluid">
      <div class="row-fluid">
        ...
        <?php
		  ...
		  $getdyn = mysql_query("SELECT * FROM archives WHERE hash='$hashcode'");
		  $rowdyn = mysql_fetch_array($getdyn);
		  $id = $rowdyn['id'];
		  $contents = $rowdyn['contents'];
		  $date = $rowdyn['date'];
		...
		?>
          ...
          <div class="hero-unit">
          <h3>Contents for <?php echo $date; ?></h3>
           <div style="padding: 10px; width: 1000px; font-family: Verdana, Geneva, sans-serif; font-size:13px; margin:10px 0px 10px 10px;"><?php echo $msg; ?></div>   
            ...
            <form class="form-signin" method="post" action="dyn-edit.php?code=<?php echo $hashcode; ?>&user=<?php echo $user; ?>">
            
            <input type="hidden" name="hashdyn" id="hashdyn" value="<?php echo $hashcode; ?>" />
            
             <label>Date</label><textarea class="input-block-level" name="date" id="date"><?php echo $date; ?></textarea>
               
             <label>Contents</label><textarea style="width:100%; height:300px;" name="content" id="content"><?php echo $contents; ?></textarea>
