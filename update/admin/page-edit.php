...
if ($_POST['parse_var'] == "new"){
	$hashpe = $_POST['hashpage'];
	$ptitlee = $_POST['ptitle'];
	$titlepe = $_POST['title'];
	$titlepe = mysql_real_escape_string(stripslashes($titlepe));
	$tagspe = $_POST['tags'];
	$tagspe = mysql_real_escape_string(stripslashes($tagspe));
	$descriptionpe = $_POST['description'];
	$descriptionpe = mysql_real_escape_string(stripslashes($descriptionpe));
	
	
	$update = mysql_query("UPDATE contentsupdate SET metatitle='$titlepe', metatags='$tagspe', metadescription='$descriptionpe' WHERE hash='$hashpe'");
	if($update){
	$msg = "S.E.O has been applied to <font color=\"#FF0000\">".stripslashes($ptitlee)."</font>. <a href=\"page.php\"><button class=\"btn btn-large btn-primary\" type=\"submit\" id=\"update\">Edit other Geography Pages</button></a>";
	}
	else{
...
<title>Add S.E.O Techniques - Geography Category</title>
    ...
    <div class="container-fluid">
      <div class="row-fluid">
        ...
        <?php
		  ...
		  $getpageedit = mysql_query("SELECT * FROM contentsupdate WHERE hash='$hashcode'");
		  $rowgetpageedit = mysql_fetch_array($getpageedit);
		  $idpage = $rowgetpageedit['id'];
		  $ptitle = $rowgetpageedit['title'];
		  $mtitle = $rowgetpageedit['metatitle'];
		  $mtags = $rowgetpageedit['metatags'];
		  $mdescription = $rowgetpageedit['metadescription'];
		  
		?>
        
          <div class="hero-unit">
          <h3>S.E.O for <?php echo $ptitle; ?></h3>
           <div style="padding: 10px; width: 850px; font-family: Verdana, Geneva, sans-serif; font-size:13px; margin:10px 0px 10px 10px;"><?php echo $msg; ?></div>   
            ...
            <form class="form-signin" method="post" action="page-edit.php?code=<?php echo $hashcode; ?>&user=<?php echo $user; ?>">
            
            <input type="hidden" name="hashpage" id="hashpage" value="<?php echo $hashcode; ?>" />
            <input type="hidden" name="ptitle" id="ptitle" value="<?php echo $ptitle; ?>" />
            
             <label>Meta Title</label><input type="text" class="input-block-level" name="title" id="title" value="<?php echo $mtitle; ?>" style="width:600px; padding:5px; font-size:13px;">
             
             <label>Meta Keywords</label><textarea style="width:100%; height:100px;" name="tags" id="tags"><?php echo $mtags; ?></textarea>
              
             <label>Meta Description</label><textarea style="width:100%; height:100px;" name="description" id="description"><?php echo $mdescription; ?></textarea>
