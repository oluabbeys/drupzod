...
<div id="content_holder_contents">
                ...
                <!--SAMPLE DIV-->
                	<div id="s_sample2">
                    	<!--Article title-->
                    	<div style="width:525px; height:40px; float:left; margin-right:10px;"><a href="http://zodml.org">Home</a> > <a href="http://zodml.org/Nigeria">Nigeria</a> > <a href="http://www.zodml.org/Nigeria/nigeria_people_category_articles.php">People</a> ><?php echo $row4['title']; ?></div> 
	...
	window.open("nigeria-comment.php?code=<?php echo $row4['id']; ?>","_blank","menubar=no, scrollbar=no, resizable=no, width=400, height=350");
                    ...
                    <div style="margin:20px 0px 0px 10px; width:260px; border:solid 1px #CCC; line-height:30px;">
                        ...
                        <?php
							...
							$rel = mysql_query('SELECT title, contents, category, searchtitle, hash FROM articles WHERE category="People" AND id!=".$id." ORDER BY title ASC');
							//$rel = mysql_query('SELECT * FROM articles WHERE title!="$title" AND category="People" ORDER BY title ASC');
							...
							while($row5 = mysql_fetch_array($rel)){
								...
								echo "<li><a href=\"nigeria_people_content_articles.php?article_id=$rel_title\">$rel_title</a></li>";
